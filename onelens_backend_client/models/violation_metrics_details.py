...
class ViolationMetricsDetails(BaseModel):
    ...
    filter: Optional[List[MetricChartFilters]] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if (
            self.chart_data_tooltip is None
            and "chart_data_tooltip" in self.model_fields_set
        ):
        ...
        # and model_fields_set contains the field
        if (
            self.look_back_period is None
            and "look_back_period" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "look_back_period": MetricsLookBackPeriod.from_dict(
                    obj["look_back_period"]
                )
                if obj.get("look_back_period") is not None
                else None,
                "filter": [
                    MetricChartFilters.from_dict(_item) for _item in obj["filter"]
                ]
                if obj.get("filter") is not None
                else None,
                "threshold": MetricsThreshold.from_dict(obj["threshold"])
                if obj.get("threshold") is not None
                else None,
                "query": MetricsQueryOutput.from_dict(obj["query"])

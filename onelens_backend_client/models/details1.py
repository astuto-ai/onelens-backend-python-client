...
DETAILS1_ANY_OF_SCHEMAS = ["TenantAnomalyTicketDetailsMixin", "TenantPolicyTicketDetailsMixin"]
...
class Details1(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, TenantPolicyTicketDetailsMixin):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TenantPolicyTicketDetailsMixin`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Details1 with anyOf schemas: TenantAnomalyTicketDetailsMixin, TenantPolicyTicketDetailsMixin. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[TenantPolicyTicketDetailsMixin] = None
        try:
            instance.actual_instance = TenantPolicyTicketDetailsMixin.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Details1 with anyOf schemas: TenantAnomalyTicketDetailsMixin, TenantPolicyTicketDetailsMixin. Details: " + ", ".join(error_messages))
        else:
            return instance

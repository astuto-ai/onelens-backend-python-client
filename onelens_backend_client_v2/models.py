...
#   filename:  <stdin>
#   timestamp: 2025-06-22T09:14:41+00:00
...
class PipelineIntermediateStage(str, Enum):
    SORTER = "SORTER"
    PARSER = "PARSER"
    FILTER = "FILTER"
    LOADER = "LOADER"
...
class PipelineRunStatusExtended(str, Enum):
    SUCCESS = "SUCCESS"
    FAILED = "FAILED"
    NOT_STARTED = "NOT_STARTED"
    IN_PROGRESS = "IN_PROGRESS"
...
class LogPipelineIntermediateStageRequest(BaseModel):
    stage: PipelineIntermediateStage = Field(
        ..., description="The intermediate stage of the pipeline."
    )
    status: PipelineRunStatusExtended = Field(
        ..., description="Status of the pipeline stage."
    )
    details: Optional[Dict[str, Any]] = Field(
        None, description="details of the pipeline stage.", title="Details"
    )
    agent_run_id: UUID = Field(
        ..., description="ID of the agent run.", title="Agent Run Id"
    )
    tenant_id: UUID = Field(..., description="ID of the tenant.", title="Tenant Id")
    ol_cluster_id: str = Field(
        ..., description="Unique ID of the cluster.", title="Ol Cluster Id"
    )
    run_type: str = Field(
        ..., description="Type of the run (e.g., PIPELINE).", title="Run Type"
    )
    from_timestamp: datetime = Field(
        ..., description="Start time of the data range.", title="From Timestamp"
    )
    to_timestamp: datetime = Field(
        ..., description="End time of the data range.", title="To Timestamp"
    )
...
class GetClusterStatsRequest(BaseModel):
    ...
    use_cur_aggregate: Optional[bool] = Field(
        False,
        description="Whether to force CUR aggregate data processor (legacy – ignored by v2 handler).",
        title="Use Cur Aggregate",
    )
...
class CreateCustomTenantTicketRequestMixin(BaseModel):
    ...
    last_run_id: Optional[UUID4] = Field(
        "f92b5817-4e40-45cb-b343-26a3f779e74a",
        description="Id of the last policy violation/anomaly run",
        title="Last Run Id",
    )
    last_run_at: Optional[datetime] = Field(
        "2025-06-22T14:41:37.707903",
        description="Datetime of the last policy violation/anomaly run",
        title="Last Run At",
    )
    first_run_at: Optional[datetime] = Field(
        "2025-06-22T14:41:37.707916",
        description="Datetime of the first policy violation/anomaly run",
        title="First Run At",
    )
    created_by: Optional[UUID4] = Field(

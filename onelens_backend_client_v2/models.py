...
#   filename:  <stdin>
#   timestamp: 2024-07-30T06:05:29+00:00
...
from pydantic import UUID4, UUID5, AnyUrl, BaseModel, EmailStr, Extra, Field, constr
...
class Auth0UserConnection(BaseModel):
    __root__: str = Field(
        "Username-Password-Authentication", const=True, title="Auth0UserConnection"
    )
...
class Granularity(BaseModel):
    __root__: Any = Field("days", const=True, title="Granularity")
...
class HierarchySharedNodeStrategy(BaseModel):
    __root__: str = Field("EQUAL", const=True, title="HierarchySharedNodeStrategy")
...
class ServiceCatalog(BaseModel):
    ...
    features: Optional[Features] = Field(
        default_factory=lambda: Features.parse_obj(
...
class TicketUnassignment(BaseModel):
    __root__: str = Field("UNASSIGNED", const=True, title="TicketUnassignment")
...
class OnelensModelsServiceInterfacesTenantDataMetricsServiceOperator(BaseModel):
    __root__: str = Field("equals", const=True, title="Operator")
...
class OnelensModelsServiceInterfacesTenantMetadataCommonsHierarchyNodeCategory2(
    ...
    __root__: OnelensModelsServiceInterfacesTenantMetadataCommonsHierarchyNodeCategory1
...
class CreateAuth0UserRequest(BaseModel):
    ...
    connection: Optional[Auth0UserConnection] = Field(
        default_factory=lambda: Auth0UserConnection.parse_obj(
...
class GetActionTypeRequest(BaseModel):
    pagination: Optional[PaginationParams] = Field(
        default_factory=lambda: PaginationParams.parse_obj(
            {"page": 1, "page_size": 10}
        ),
        description="Pagination parameters for the request.",
    )
    filters: Optional[ActionTypeFilters] = Field(
        default_factory=lambda: ActionTypeFilters.parse_obj(
            {"ids": [], "services": []}
        ),
...
class GetHierarchyRequest(BaseModel):
    filters: Optional[GetHierarchyFilters] = Field(
        default_factory=lambda: GetHierarchyFilters.parse_obj({"state": "ACTIVE"}),
...
class GetRecommendationUnitRequest(BaseModel):
    ...
    filters: Optional[RecommendationUnitFilters] = Field(
        default_factory=lambda: RecommendationUnitFilters.parse_obj(
            {
                "ids": [],
                "services": [],
                "action_type_ids": [],
...
class GetTenantPoliciesAPIRequest(BaseModel):
    ...
    filters: Optional[TenantPolicyFilters] = Field(
        default_factory=lambda: TenantPolicyFilters.parse_obj(
            {
                "parent_ptp_ids": [],
                "categories": [],
                "providers": [],
...
class GetPolicyTemplatesRequest(BaseModel):
    ...
    filters: Optional[PolicyTemplateFilters] = Field(
        default_factory=lambda: PolicyTemplateFilters.parse_obj(
            {
                "parent_ptp_ids": [],
                "states": ["ACTIVE"],
                "categories": [],
...
class GetTenantAnomalySettingsAPIRequest(BaseModel):
    ...
    filters: Optional[TenantAnomalySettingFilters] = Field(
        default_factory=lambda: TenantAnomalySettingFilters.parse_obj(
            {"states": ["ACTIVE"]}
        ),
...
class GetTenantPolicySettingsAPIRequest(BaseModel):
    ...
    filters: Optional[TenantPolicySettingsFilters] = Field(
        default_factory=lambda: TenantPolicySettingsFilters.parse_obj(
...
class GetTenantTicketsAPIRequest(BaseModel):
    ...
    filters: Optional[TenantTicketFilters] = Field(
        default_factory=lambda: TenantTicketFilters.parse_obj(
            {
                "monitor_ids": [],
                "ticket_categories": [],
                "states": [],

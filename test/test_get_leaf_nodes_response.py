# coding: utf-8

"""
    onelens-backend

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from onelens_backend_client.models.get_leaf_nodes_response import GetLeafNodesResponse

class TestGetLeafNodesResponse(unittest.TestCase):
    """GetLeafNodesResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GetLeafNodesResponse:
        """Test GetLeafNodesResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GetLeafNodesResponse`
        """
        model = GetLeafNodesResponse()
        if include_optional:
            return GetLeafNodesResponse(
                nodes = [
                    onelens_backend_client.models.hierarchy_node_entity_dto.HierarchyNodeEntityDTO(
                        id = '', 
                        name = '', 
                        category = null, 
                        resource_filters = [
                            onelens_backend_client.models.hierarchy_node_resource_filters.HierarchyNodeResourceFilters(
                                key = null, 
                                field = '', 
                                operator = '', 
                                value = null, )
                            ], 
                        resource_filter_expression = '', 
                        is_shared = True, 
                        attribution_details = onelens_backend_client.models.hierarchy_node_attribution_details.HierarchyNodeAttributionDetails(
                            nodes = [
                                56
                                ], 
                            strategy = null, ), 
                        state = null, 
                        sql_filter = '', )
                    ]
            )
        else:
            return GetLeafNodesResponse(
                nodes = [
                    onelens_backend_client.models.hierarchy_node_entity_dto.HierarchyNodeEntityDTO(
                        id = '', 
                        name = '', 
                        category = null, 
                        resource_filters = [
                            onelens_backend_client.models.hierarchy_node_resource_filters.HierarchyNodeResourceFilters(
                                key = null, 
                                field = '', 
                                operator = '', 
                                value = null, )
                            ], 
                        resource_filter_expression = '', 
                        is_shared = True, 
                        attribution_details = onelens_backend_client.models.hierarchy_node_attribution_details.HierarchyNodeAttributionDetails(
                            nodes = [
                                56
                                ], 
                            strategy = null, ), 
                        state = null, 
                        sql_filter = '', )
                    ],
        )
        """

    def testGetLeafNodesResponse(self):
        """Test GetLeafNodesResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
